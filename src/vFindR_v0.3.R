...
if (Sys.info()["user"] == "dbuckley") {
  ...
  vir.idx.base <- "/Users/dbuckley/Desktop/salhia_lab/genomes/refseq_all_virus/refseq_all_virus.fa"
  ...
  # first alignment
  # aln.hg.1.temp <- paste0(output.stub, "_", ref.species, ".bam")
...
cmds['get.unmapped.first'] <- paste0("{ ", 
                                     paste("samtools view -H ", aln.hg.1.out), " && ",
...
cmds['index'] <- paste0("for i in ~/Desktop/salhia_lab/vFindeR/test_output/*bam; do samtools index $i; done")
cmds['idxstats'] <- paste0("for i in ~/Desktop/salhia_lab/vFindeR/test_output/*bam; do samtools idxstats $i > $i.idxstats; done")
write.table(addEcho(cmds), "commands.sh", row.names = F, col.names = F, quote = F)
